/* winodw.onload로 로드 되면 달력 출력되게 하기 */
    ...
    for(let i = 0; i < weekCount; i++) {
        ...
        for(let y = 0; y < 7; y++) {
            ...
            if(StartDay <= candleXYZ && candleDay < lastMonthDayDate) {
                candleDay++;
                html += "<sapn style='display: block; width: 100%; height: 100%'>" + candleDay + "</span>";
            }
            html += "</td>"; 
            candleXYZ++;
        }
...
function right_Btn() {
    ...
    // 월이 12월을 넘어가면 (12보다 크면), 월을 1월로 초기화하고 년도도 증가
    if (Month > 12) {
        Month = 1;
        Year++; // 월이 초기화되면 연도도 증가시킵니다.
    }

    // "load_Candle()" 함수를 호출하여 새로운 Month 값으로 달력을 업데이트.
    load_Candle(Year, Month);
}
...
function left_Btn() {
    ...
    // 월이 01월보다 아래로 내려가면 (1보다 작으면), 월을 12월로 초기화하고 년도도 차감.
    if (Month < 1) {
        Month = 12;
        Year--; // 월이 초기화되면 연도도 증가시킵니다.
    }
